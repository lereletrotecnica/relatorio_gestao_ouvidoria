# Estatísticas 2021 {#sec-dados-ouvidoria-2021}
...
Principalmente, objetiva-se quantificar o trabalho exercido pela Onasp. Especificamente, este capítulo visa gerar índices qualitativos / quantitativos para apoio a tomada de decisão dos gestores e supervisionar o andamento das atividades de
ouvidoria em âmbito nacional.

São analisados - numericamente e graficamente - os seguintes aspectos: características dos processos recebidos ([seção @sec-caracteristicas-processos-2021]); demandas tipificadas como solicitações ([seção @sec-solicitacoes-2021]); denúncias ([seção @sec-denuncias-2021]); reclamações ([seção @sec-reclamacoes-2021]); sugestões ([seção @sec-sugestoes-2021]); elogios ou agradecimentos ([seção @sec-elogios-2021]); e evolução dos índices gerados para controle do tempo de atendimento dos processos ([seção @sec-tempos-2021]).

Para ampliar o poder de visualização do trabalho, a metodologia de coleta de dados foi melhorada e novas informações foram agregadas em 2021. Como resultado, têm-se informações sobre a forma como os usuários acessam a ouvidoria ([seção @sec-meios-acesso-ouvidoria-2021]) e sobre a aptidão e tratamento dos requerimentos ([seção @sec-aptidao-2021]). Para o restante do quesitos, manteve-se a metodologia de análise das informações.
...
## Sobre os meios de acesso à Ouvidoria {#sec-meios-acesso-ouvidoria-2021}
...
## Sobre a aptidão ao tratamento das demandas {#sec-aptidao-2021}
...
O gráfico na [figura @fig-aptidao1-2021] traz os motivos da inaptidão das demandas e suas respectivas grandezas em valores absolutos e relativos.

```{r}
#| label: fig-aptidao1-2021
#| fig-cap: >  
...
## Características dos processos {#sec-caracteristicas-processos-2021}
...
Verifica-se a relevância numérica apoiada nos valores absolutos e relativos das ocorrências de cada tipo de demanda ([seção @sec-tipos-processos-2021]).

Necessário constar que as análises realizadas, a partir deste tópico, se referem às demandas consideradas aptas.

### Tipos de processos {#sec-tipos-processos-2021}
...
O gráfico na [figura @fig-tipo-demanda-2021] deixa clara a diferenciação quantitativa das características dos processos contendo solicitações. Sendo 
`r max(dados_gerais_tipos_demandas$qtd)` 
demandas tipificadas dessa maneira, ou seja, 
`r str_c(max(dados_gerais_tipos_demandas$porcentagem),"%")` 
são consideradas como **pedidos** de diversos modos.

```{r}
#| label: fig-tipo-demanda-2021
#| fig-cap: >  
...
## Solicitações {#sec-solicitacoes-2021}
...
Na [seção @sec-tipos-solicitacoes-2021] lista-se o rol de solicitações de acordo com a característica do pedido.

### Tipos de solicitações {#sec-tipos-solicitacoes-2021}
...
Conforme já citado na [seção @sec-tipos-processos-2021], as solicitações representam `r str_c(max(dados_gerais_tipos_demandas$porcentagem),"%")` do total de demandas recebidas pela Onasp.

A próxima tabela lista as quantidades de solicitações separadas por seus respectivos motivos.

```{r}
#| label: tbl-tipos-solicitacoes-2021
#| tbl-cap: >
...
O quadro na [figura @fig-graf-tipos-solicitacoes-2021] mostra os `r nrow(solicitacoes3)` tipos mais recorrentes de solicitações. Nota-se a discrepância percentual das solicitações de **Assistência Jurídica** com `r str_c(max(solicitacoes3$porcentagem),"%")` de incidência.

```{r}
#| label: fig-graf-tipos-solicitacoes-2021
#| fig-cap: >  
...
Esta seção traz um resumo numérico do comportamento dos tipos de denúncia (@sec-tipos-denuncias-2021) e o perfil do cargo ocupado pelo denunciado ([seção @sec-denuncias-denunciado-2021]).

### Tipos de Denúncias {#sec-tipos-denuncias-2021}
...
```{r}
#| label: tbl-tipos-denuncias-2021
#| tbl-cap: >
...
O gráfico na [figura @fig-graf-tipos-denuncias-2021] mostra os `r nrow(head(denuncias1,n=10))` tipos de denúncias mais recorrentes. Nota-se que, relatos de **maus tratos, torturas e ameaças** são comuns relativizando `r str_c(denuncias1$porcentagem[1],"%")` dos casos.

```{r}
#| label: fig-graf-tipos-denuncias-2021
#| fig-cap: >  
...
### Denúncias - Perfil dos denunciados {#sec-denuncias-denunciado-2021}
...
```{r}
#| label: tbl-denuncias-denunciados-2021
#| tbl-cap: >
...
Já a [figura @fig-graf-denunciados-2021] evidencia graficamente as informações da última tabela.

```{r}
#| label: fig-graf-denunciados-2021
#| fig-cap: >  
...
## Reclamações {#sec-reclamacoes-2021}
...
Na [seção @sec-tipos-reclamacoes-2021] tem-se a lista de características das reclamações.

### Tipos de reclamações {#sec-tipos-reclamacoes-2021}
...
```{r}
#| label: tbl-tipos-reclamacoes-2021
#| tbl-cap: >
...
O gráfico na [figura @fig-graf-tipos-reclamacoes-2021] mostra os `r nrow(head(reclamacoes1,n=10))` tipos de reclamações mais recorrentes.

```{r}
#| label: fig-graf-tipos-reclamacoes-2021
#| fig-cap: >  
...
### Tipos de Sugestões {#sec-tipos-sugestoes-2021}
...
```{r}
#| label: tbl-tipos-sugestoes-2021
#| tbl-cap: >
...
O gráfico na [figura @fig-graf-tipos-reclamacoes-2021] mostra os `r nrow(sugestoes1)` tipos de reclamações mais recorrentes.

```{r}
#| label: fig-graf-tipos-sugestoes-2021
#| fig-cap: >  
...
## Elogios ou agradecimentos {#sec-elogios-2021}
...
A [seção @sec-tipos-elogios-2021] quantifica os assuntos elogiados / agradecidos.

### Tipos de elogios ou agradecimentos {#sec-tipos-elogios-2021}
...
O gráfico na [figura @fig-graf-tipos-elogios-2021] mostra os `r nrow(elogios1)` tipos de elogios / agradecimentos mais recorrentes.

```{r}
#| label: fig-graf-tipos-elogios-2021
#| fig-cap: >  
...
## Tempo de atendimento {#sec-tempos-2021}
...
A [seção @sec-tempo-medio-2021] analisa os valores e evoluções do tempo médio de atendimento das demandas, enquanto a [seção @sec-tempo-medio-2021] repete as análises, porém utilizando a mediana como medida resumo dos dados.
...
### Médias dos tempos de atendimento {#sec-tempo-medio-2021}

Agora, a [figura @fig-evolucao-tempo-media-2021] mostra a evolução das médias mensais dos tempos de atendimento.

```{r}
#| label: fig-evolucao-tempo-media-2021
#| fig-cap: >
...
### Medianas dos tempos de atendimento {#sec-tempo-mediano-2021}

O gráfico na [figura @fig-evolucao-tempo-mediana-2021] repete a análise anterior, porém utilizando a mediana como índice de medição.

```{r}
#| label: fig-evolucao-tempo-mediana-2021
#| fig-cap: >
